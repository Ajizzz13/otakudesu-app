...
async function load(url) {
  return cheerio.load(await fetchHtml(url));
}
...
async function getEpisodeStream(slug) {
  ...
  if (defaultStreamingUrl) {
    const cached = streamCache.get(defaultStreamingUrl);
    if (cached && Date.now() - cached.at < 30 * 60 * 1000) {
      streams = cached.streams;
    } else {
      try {
        streams = await resolveBloggerStreams(defaultStreamingUrl);
        streamCache.set(defaultStreamingUrl, { streams, at: Date.now() });
      } catch { streams = []; }
    }
  }

  return { title, defaultStreamingUrl, mirrors, navigation: nav, sizes: uniqueSizes, streams };
}

async function resolveBloggerStreams(videoUrl) {
  const html = await fetchHtml(videoUrl);
  const sid = (html.match(/"FdrFJe":"([^"]+)"/) || [])[1];
  const bl = (html.match(/boq_bloggeruiserver_[^"' ,]+/) || [])[0];
  if (!sid || !bl) throw new Error('no blogger sid');
  const token = (videoUrl.match(/token=([^&]+)/) || [])[1];
  if (!token) throw new Error('no token');

  const rpc = `[[["WcwnYd","[\\"${token}\\\"]",null,"generic"]]]`;
  const res = await fetch(
    `https://www.blogger.com/_/BloggerVideoPlayerUi/data/batchexecute?rpcids=WcwnYd&source-path=%2Fvideo.g&f.sid=${encodeURIComponent(sid)}&bl=${encodeURIComponent(bl)}&hl=en-US&rt=c`,
    {
      method: 'POST',
      headers: {
        'User-Agent': UAS[0],
  ...
  const found = [];
  const urlRe = /"(https:\/\/[^"]+googlevideo\.com[^"]+)"/g;
  let m;
  while ((m = urlRe.exec(body))) {
    const url = m[1]
      ...
      .replace(/\\&/g, '&')
      .replace(/\\+$/, '');
    if (!found.some((f) => f.url === url)) found.push(url);
  }

  const labels = { '18': '360p', '22': '720p', '37': '1080p' };
  return found.map((url) => {
    const itag = (url.match(/[?&]itag=(\d+)/) || [])[1];
    return { quality: (itag && labels[itag]) || (itag ? 'itag ' + itag : 'auto'), url };
  });
}
...
async function resolveMirror(payload) {
  ...
  const res = await fetch(`${BASE}/wp-admin/admin-ajax.php`, {
    ...
    body: `action=2a3505c93b0035d3f455df82bf976b84&nonce=${encodeURIComponent(nonce)}&id=${encodeURIComponent(id)}&i=${encodeURIComponent(i)}&q=${encodeURIComponent(q)}`,
  });
  const data = await res.json();

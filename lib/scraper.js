...
async function getEpisodeStream(slug) {
  ...
  return { title, defaultStreamingUrl, mirrors, navigation: nav, sizes: uniqueSizes, streams: await getDirectStreams(defaultStreamingUrl) };
}

const streamCache = new Map();
async function getDirectStreams(iframeSrc) {
  if (!iframeSrc) return [];
  const cached = streamCache.get(iframeSrc);
  if (cached && Date.now() - cached.at < 30 * 60 * 1000) return cached.streams;
  try {
    const streams = await resolveBloggerStreams(iframeSrc);
    streamCache.set(iframeSrc, { streams, at: Date.now() });
    return streams;
  } catch {
    return [];
  }

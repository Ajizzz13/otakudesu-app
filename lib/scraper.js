...
async function getEpisodeStream(slug) {
  ...
  return { title, defaultStreamingUrl, mirrors, downloads, navigation: nav };
}
...
async function searchAnime(q) {
  const $ = await load(`${BASE}/?s=${encodeURIComponent(q)}`);
  const items = [];
...
async function getAnimeList(page = 1) {
  const $ = await load(`${BASE}/anime-list/page/${page}/`);
  const items = [];
  $('.anime-info tbody tr').each((_, tr) => {
    const $a = $(tr).find('td a').first();
    const url = $a.attr('href');
    if (!url) return;
    items.push({ title: $a.text().trim(), slug: slugOf(url), url });
  });
  let totalPages = page;
  $('.pagination a').each((_, a) => {
...
async function getSchedule() {
  ...
  $('.kgjdwl').each((_, el) => {
    const $el = $(el);
    const day = $el.find('.kgjdwl-judul').text().trim() || 'Hari';
    const list = [];
    $el.find('.kglist321 li a, .kglist321 li').each((_, li) => {
      const $a = $(li).find('a').first();
      const $li = $(li);
      if ($a.length) list.push({ title: $a.text().trim(), url: $a.attr('href') });
      else if ($li.text().trim()) list.push({ title: $li.text().trim(), url: '' });
    });
